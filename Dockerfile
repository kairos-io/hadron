...
RUN ./configure --quiet ${COMMON_CONFIGURE_ARGS} \
    ...
    --disable-multilib
RUN . /arch-env.sh && make ARCH="${ARCH}" CROSS_COMPILE="${TARGET}-" -j${JOBS}
RUN . /arch-env.sh && make ARCH="${ARCH}" CROSS_COMPILE="${TARGET}-" -j${JOBS} DESTDIR=/gdb install install-gdbserver
