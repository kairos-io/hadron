...
- We strip at the end, but fi you build with meson, there is a swtich to do it automatically which works nicely `-Dstrip=true` and valid for all meson builds
- If you have a single dep only used by a single program, build it static. If its not used by any other thing int he final system, its much nicer to incrust whatever is needed in the final app rather than have the full shared lib which is only used by a single program.
...
- Comrpess everything with upx `--best --lzma` for final releases
